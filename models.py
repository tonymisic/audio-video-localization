...
class Text(nn.Module):
    def __init__(self, text_size=768, out=64, normalize=True):
        super(Text, self).__init__()
        self.normalize = normalize
        self.layer1 = nn.Linear(text_size, 512)
        self.layer2 = nn.Linear(512, 256)
        self.layer3 = nn.Linear(256, out)
    
    def forward(self, text):
        text = F.relu(self.layer1(text))
        text = F.relu(self.layer2(text))
        text = self.layer3(text)
        if self.normalize:
